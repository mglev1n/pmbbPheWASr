...
```{r example-pmbb_format_phecodes}
#' \dontrun{
...
#' @param phecode_file Path to PMBB phenotype file used to run PheWAS
#' @param covariate_file Path to PMBB covariate file used to run PheWAS
#' @param demographics_file Path to PMBB demographics file used to run PheWAS
#' @param populations A character vector of populations to run PheWAS on. Default is `c("EUR", "AFR", "ALL")`
...
#' 
#' @export
run_pmbb_phewas <- function(mask_genotypes_list, phecode_file, covariate_file, demographics_file, populations = c("EUR", "AFR", "ALL", phenotype_case_count = 2, ...)) {
  # Check if mask_genotypes_list is a list
  if (!is.list(mask_genotypes_list)) {
    cli::cli_abort("`mask_genotypes_list` must be a list")
  }

  # Check if phenotype_file exists
  if (!file.exists(phecode_file)) {
    cli::cli_abort("{phenotype_file} does not exist")
  }

  # Check if covariate_file exists
  if (!file.exists(covariate_file)) {
    cli::cli_abort("{covariate_file} does not exist")
  }

  # Check if demographics_file exists
  if (!file.exists(demographics_file)) {
    cli::cli_abort("{demographics_file}  does not exist")
  }

  # Check if populations is a character vector
  if (!is.character(populations)) {
    cli::cli_abort("populations must be a character vector")
  }

  # Check if populations is not empty
  if (length(populations) == 0) {
    cli::cli_abort("populations must not be empty")
  }

  # Read phenotype file and apply phenotype_case_count
  
}
```
  
```{r example-run_pmbb_phewas}
#' \dontrun {
run_pmbb_phewas(
  ...
  covariate_file = "/project/PMBB/PMBB-Release-2020-2.0/Phenotype/2.3/PMBB-Release-2020-2.3_covariates.txt",
  demographics_file = "/project/PMBB/PMBB-Release-2020-2.0/Phenotype/2.3/PMBB-Release-2020-2.3_phenotype_demographics.txt",
  populations = c("EUR", "AFR", "ALL")
)
#' }
