...
The `run_phewas_report` function is a wrapper around the `run_pmbb_phewas` function that generates an HTML report of the results.

```{r function-run_phewas_report}
#' Title
#' 
#' Description
#' 
#' @param mask_output_path (optional) Path to save the genotype output for each mask (default: NULL)
#' @param phewas_output_path (optional) Path to save the PheWAS output dataframe (default: NULL)
...
#' 
#' @export
run_phewas_report <- function(gene, annotation_file, gene_col, masks, mask_operator, variant_id_col, effect_allele_col, plink_bin, bfile, phecode_file, covariate_files, populations, covariate_population_col, covariate_cols, mask_output_path = NULL, phewas_output_path = NULL, report_output_path = NULL, ...) {
  ...
  quarto::quarto_render(
    ...
    execute_params = list(
      ...
      gene_names = gene_names_collapsed
    ),
    output_file = output_file
  )
  
  # Move the generated report to the specified report_output_path
  if (!is.null(report_output_path)) {
    fs::dir_create(fs::path_dir(report_output_path))
    fs::file_move(output_file, report_output_path)
